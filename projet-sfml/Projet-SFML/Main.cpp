...
int main() {
    ...
    if (!crossTexture.loadFromFile("assets/cross.jpg") || !circleTexture.loadFromFile("assets/circle.png")) {
    ...
    introText.setFillColor(Color::White);  // Changement de couleur pour le texte
    ...
    endText.setPosition(300, 250);  // Positionner le texte de fin
    endText.setFillColor(Color::White);  // Texte en blanc
    ...
    while (window.isOpen()) {
        ...
        if (Mouse::isButtonPressed(Mouse::Left)) {
            ...
            if (gameEnded && restartButton.getGlobalBounds().contains(localPosition.x, localPosition.y)) {
                ...
                for (int i = 0; i < GRID_SIZE; ++i) {
                    for (int j = 0; j < GRID_SIZE; ++j) {
                        if (grid[i][j].shape.getGlobalBounds().contains(localPosition.x, localPosition.y) && grid[i][j].player == Player::None) {
